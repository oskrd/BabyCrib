...
char processRequest(String & req) {postt=0;
...
void writeResponse(WiFiClient& client, JsonObject& json) {
  ...
  client.println(headers);
    ...
    delay(333);
  #endif
  if(postt==0){if(json.size()<1){Serial.println("-------------------------Error detectado-------------------");delay(500);ESP.reset();}else{json.prettyPrintTo(client);}}
  else{postt=0;}
